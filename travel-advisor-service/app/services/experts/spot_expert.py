...
class SpotExpert(BaseExpert):
    ...
    SPOT_PROVINCE_IDS = {
        ...
        "da lat": "da-lat",
        "dalat": "da-lat",
        "lâm đồng": "da-lat",  # Province name -> city in spots DB
        "lam dong": "da-lat",
        
        "hội an": "hoi-an",
        "hoi an": "hoi-an",
        "hoian": "hoi-an",
        "quảng nam": "hoi-an",  # Province -> city
        "quang nam": "hoi-an",
    ...
    SYNONYMS = {
        ...
        "núi": ["núi", "đồi", "mountain", "leo núi", "trekking"],
        "chùa": ["chùa", "đền", "temple", "pagoda", "tự viện", "tâm linh", "tu viện"],
        "thác": ["thác", "thác nước", "waterfall"],
        "hồ": ["hồ", "lake", "hồ nước"],
        "phố cổ": ["phố cổ", "old town", "phố"],
        "công viên": ["công viên", "park", "vườn"],
        "bảo tàng": ["bảo tàng", "museum", "triển lãm"],
        "cảnh đẹp": ["cảnh đẹp", "view", "ngắm cảnh", "check-in"],
        "tâm linh": ["tâm linh", "chùa", "đền", "temple", "pagoda", "tự viện", "linh thiêng"]
    }
    ...
    def __init__(self, mongo_client=None, vector_client=None, llm_client=None, embedding_service=None):
        ...
        if self.embedding:
            logger.info("✅ SpotExpert with semantic search enabled")
        else:
            logger.info("⚠️  SpotExpert running without semantic search")
        
        # [NEW] Initialize Hybrid Search
        if SpotExpert._hybrid_search is None:
            try:
                from app.services.hybrid_search import hybrid_search_service
                SpotExpert._hybrid_search = hybrid_search_service
                logger.info("✅ SpotExpert: Hybrid Search initialized")
            except Exception as e:
                logger.error(f"❌ SpotExpert failed to init Hybrid Search: {e}")
                SpotExpert._hybrid_search = False  # Mark as attempted but failed
    ...
    def _normalize_location_for_spots(self, location: str) -> str:
        ...
        if location_lower in self.SPOT_PROVINCE_IDS:
            ...
            logger.info(f"📍 Spot location mapping: '{location}' -> '{province_id}'")
            return province_id
        ...
        clean_location = re.sub(r"\s*(tỉnh|thành phố|tp\.?|city)\s*", "", location_lower, flags=re.IGNORECASE)
        ...
        if clean_location in self.SPOT_PROVINCE_IDS:
            ...
            logger.info(f"📍 Spot location mapping (cleaned): '{location}' -> '{province_id}'")
            return province_id
        ...
        logger.info(f"📍 Spot location fallback: '{location}' -> '{province_id}'")
        return province_id
    ...
    def execute(self, query: str, parameters: Dict[str, Any]) -> ExpertResult:
        ...
        try:
            ...
            logger.info(f"🔍 SpotExpert: query='{query}', original='{original_query[:50]}...', province={province_id}")
            
            # [NEW] Try Hybrid Search first if available
            if SpotExpert._hybrid_search and SpotExpert._hybrid_search is not False:
                logger.info("   🚀 Using Hybrid Search for spots...")
                try:
                    spots = SpotExpert._hybrid_search.search_spots(
                        query=original_query,  # Use original query for better semantic matching
                        province_id=province_id,
                        limit=limit,
                        threshold=0.25  # Ngưỡng thấp để lấy đa dạng kết quả
                    )
                    
                    if spots:
                        elapsed = time.time() - start_time
                        logger.info(f"   ✅ Hybrid Search returned {len(spots)} spots ({elapsed:.2f}s)")
            ...
            if any(word in query_for_patterns for word in ["tâm linh", "religion", "spiritual"]):
                search_terms.extend(["chùa", "đền", "temple", "pagoda", "tự viện", "linh thiêng"])
                logger.info("🔍 Detected spiritual keywords - adding temple-related terms")
            if any(word in query_for_patterns for word in ["cổ", "lịch sử", "history", "ancient"]):
                search_terms.extend(["phố cổ", "di tích", "bảo tàng", "cổ kính"])
                logger.info("🔍 Detected historical keywords - adding heritage-related terms")
            
            search_terms = list(set(search_terms))  # Deduplicate
            logger.info(f"🔍 Final search terms: {search_terms[:10]}")
            ...
            if semantic_results and keyword_results:
                ...
                logger.info(f"📊 Using hybrid reranked results ({len(results)} spots)")
            elif keyword_results:
                # Only keyword results
                results = keyword_results
                logger.info("📊 Using keyword-only results")
            ...
            if len(results) < min_expected:
                ...
                logger.info(f"📝 Recorded data gap: {location or province_id}/spots ({len(results)} < {min_expected})")
            ...
            logger.info(f"🎯 Confidence: {confidence_info['score']} ({confidence_info['level']}) - {confidence_info['reason']}")
            ...
            if need_web_search:
                logger.info(f"🌐 Low confidence ({confidence_info['score']}), activating web search...")
                try:
                    # Build context from existing results
                    context = None
                    if results:
                        context = f"Đã tìm được {len(results)} địa điểm từ database: " + \
                    ...
                    if web_result.get('answer') and web_result.get('confidence', 0) > 0.5:
                        ...
                        logger.info(f"✅ Web search successful (confidence: {web_result['confidence']})")
                        logger.info(f"📰 Sources: {', '.join([s['source'] for s in web_result.get('sources', [])])}")
                    else:
                        logger.warning("⚠️ Web search returned low quality results")
            ...
            logger.error(f"❌ SpotExpert error: {e}")
            return ExpertResult(
                expert_type=self.expert_type,
                success=False,
                data=[],
    ...
    def _expand_search_terms(self, query: str, interests: List[str], keywords: List[str]) -> List[str]:
        ...
        logger.info(f"🔍 Expanded search terms: {list(terms)[:10]}")  # Debug
        return list(terms)
        ...
        try:
            ...
            for doc in cursor:
                results.append({
                    ...
                    "cost": doc.get("cost", ""),
                    "tags": doc.get("tags", []),
                    "latitude": doc.get("latitude"),  # 🆕 NEW: For distance calculations
                    "longitude": doc.get("longitude"),  # 🆕 NEW: For distance calculations
                    "source": "mongodb"
                })
    ...
    def _get_or_create_embedded_spots(self, province_id: str, limit: int) -> List[Dict]:
        ...
        if (SpotExpert._embedded_spots_cache is not None and 
            ...
            logger.info(f"💾 Using cached embeddings for {province_id}")
        ...
        try:
            ...
            for doc in cursor:
                spots.append({
                    "id": doc.get("id"),
                    "name": doc.get("name"),
                    "latitude": doc.get("latitude"),  # 🆕 NEW
                    "longitude": doc.get("longitude"),  # 🆕 NEW
                    "province_id": doc.get("province_id"),
            ...
            logger.info(f"📦 Chunking and embedding {len(spots)} spots...")
            ...
            logger.info(f"✅ Cached {len(embedded_spots)} embedded spots for {province_id}")
            
            return embedded_spots
            
        except Exception as e:
            import traceback
            logger.error(f"❌ Error creating embedded spots: {e}")
    ...
    def _generate_summary(self, results: List[Dict], location: str) -> str:
        ...
        if not results:
            return f"Không tìm thấy địa điểm phù hợp ở {location or 'khu vực này'}"
        
        top_spots = [r.get("name", "?") for r in results[:3]]
        return f"Tìm thấy {len(results)} địa điểm tại {location or 'Việt Nam'}. Top gợi ý: {', '.join(top_spots)}"
