...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const url = `${PYTHON_BACKEND}/api/spots/by-province?slug=${encodeURIComponent(slug)}&limit=${limit}&offset=${offset}`
    ...
    return NextResponse.json({ spots: [], total: 0, province_id: slug, limit, offset, isFallback: true }, { status: 200 })
  }
}